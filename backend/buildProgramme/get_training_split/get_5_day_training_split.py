...
def get_5_day_training_split(excluded_muscle_groups, preferred_muscle_groups, sets_per_muscle):
    ...
    upper = ["chest", "middle back", "triceps", "biceps", "chest", "lats", "triceps", "biceps", "rear delt", "chest", "middle back", "triceps", "biceps"]
    lower = ["quadriceps", "hamstrings", "front delt", "lateral delt", "abdominals", "quadriceps", "hamstrings", "calves", "abdominals", "adductors", "abductors", "glutes"]

    day_1_muscle_groups = push.copy()
    day_2_muscle_groups = pull.copy()
    day_3_muscle_groups = legs.copy()
    day_4_muscle_groups = upper.copy()
    day_5_muscle_groups = lower.copy()
    ...
    if not day_3_requires_mod and len(day_3_muscle_groups) < 0.6 * len(legs):
        day_3_requires_mod = True
    if not day_4_requires_mod and len(day_4_muscle_groups) < 0.6 * len(upper):
        day_4_requires_mod = True
    if not day_5_requires_mod and len(day_5_muscle_groups) < 0.6 * len(lower):
        day_5_requires_mod = True

    further_modifications_required = day_1_requires_mod or day_2_requires_mod or day_3_requires_mod or day_4_requires_mod or day_5_requires_mod
    ...
    if further_modifications_required:
        ...
        if day_2_requires_mod:
            ...
            # prioritise muscle_groups not trained in day 4 if not subject to change
            if not day_4_requires_mod:
                # if day 2 requires modification and day 4 does not, modify day 2 by prioritising muscle groups not trained in day 4
                low_priority_muscle_groups = set(day_4_muscle_groups) # O(1) lookups
                prioritised_muscle_groups = [x for x in ordered_muscle_groups if x not in low_priority_muscle_groups]
                ordered_muscle_groups = orderMuscleGroups(ordered_muscle_groups, [], prioritised_muscle_groups)
        ...
        # logic for prioritising preferred muscle groups for unmodified PUSH/PULL/LEGS UPPER/LOWER
        '''
        push = ["chest", "triceps", "front delt", "lateral delt", "chest", "triceps", "chest", "triceps", "front delt", "traps"]
        pull = ["middle back", "biceps", "lats", "rear delt", "biceps", "forearms", "middle back", "traps", "biceps", "lats"]
        legs = ["quadriceps", "hamstrings", "calves", "abdominals", "quadriceps", "hamstrings", "adductors", "abductors", "abdominals", "quadriceps", "calves", "abdominals", "glutes"]
        ...
        for muscle_group in preferred_muscle_groups:
            if muscle_group == "chest":
                ...
                day_2_muscle_groups.insert(0, muscle_group)
                day_4_muscle_groups.insert(0, muscle_group)
            elif muscle_group == "biceps" or muscle_group == "rear delt":
                day_2_muscle_groups = removeLastOccurrence(day_2_muscle_groups, muscle_group)
                day_2_muscle_groups.insert(0, muscle_group)
                day_4_muscle_groups = removeLastOccurrence(day_4_muscle_groups, muscle_group)
                day_4_muscle_groups.insert(0, muscle_group)
            elif muscle_group == "forearms" or muscle_group == "traps" or muscle_group == "lower back":
                day_2_muscle_groups.insert(0, muscle_group)
                day_4_muscle_groups.insert(0, muscle_group)
                ...
                day_3_muscle_groups.insert(0, muscle_group)
                day_5_muscle_groups.insert(0, muscle_group)
            elif muscle_group == "abductors" or muscle_group == "adductors":
                day_3_muscle_groups = removeLastOccurrence(day_3_muscle_groups, muscle_group)
                day_3_muscle_groups.insert(0, muscle_group)
                day_5_muscle_groups = removeLastOccurrence(day_5_muscle_groups, muscle_group)
                day_5_muscle_groups.insert(0, muscle_group)
    ...
    return [(day_1, day_1_requires_mod), (day_2, day_2_requires_mod), (day_3, day_3_requires_mod), 
            (day_4, day_4_requires_mod), (day_5, day_5_requires_mod)]